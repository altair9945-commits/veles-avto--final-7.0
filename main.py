# VERSION: VELES_BOT_V7_3_FIX_TON_MEMO_AUTOCHECK_2026_07_18
...
def _fetch_ton_transactions_sync(wallet: str) -> list:
    ...
    headers = {"User-Agent": "VELES-BOT/7.3"}
...
def decode_ton_comment(incoming: dict) -> str:
    direct = incoming.get("message")
    if isinstance(direct, str) and direct.strip():
        return direct.strip()

    msg_data = incoming.get("msg_data") or {}
    raw = msg_data.get("text") or ""
    if not isinstance(raw, str) or not raw.strip():
        return ""

    raw = raw.strip()
    if re.search(r"VT-\d+", raw.upper()):
        return raw
...
def parse_ton_incoming(tx: dict, wallet: str) -> dict | None:
    ...
    comment = decode_ton_comment(incoming)
...
async def process_detected_payment(context, tx: dict) -> bool:
    ...
    with db() as conn:
        ...
        if expected <= 0:
            return False
        # Запрос уже выполнен по нужному кошельку.
        # Не сравниваем строковые формы TON-адреса: bounceable и non-bounceable
        # могут отличаться текстом, но указывать на один кошелёк.
...
def main():
    ...
    logging.warning("VELES BOT v7.3 FIX TON MEMO AUTOCHECK запущен.")
