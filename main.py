# VERSION: VELES_BOT_V7_0_AUTO_PAYMENT_DIGITAL_DELIVERY_2026_07_16
...
def auto_payment_status_text() -> str:
    ...
    api = "✅ API key есть" if TONCENTER_API_KEY else "❌ нет TONCENTER_API_KEY"
    rate = ton_rub_rate()
...
async def callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if data == "main_menu":
            ...
            if new == "1" and (not TONCENTER_API_KEY or ton_rub_rate() <= 0):
                await query.answer("Сначала добавь TONCENTER_API_KEY и курс TON/₽.", show_alert=True)
            else:
...
def main():
    ...
    logging.warning("VELES BOT v7.0 AUTO PAYMENT DIGITAL DELIVERY запущен.")
